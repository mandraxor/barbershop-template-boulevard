    ...
    <main className="py-12 sm:py-20 px-3 sm:px-8 max-w-7xl mx-auto overflow-hidden">
      ...
      <FadeIn>
        <div className="text-center max-w-3xl mx-auto mb-12 sm:mb-16">
          <span className="text-[#1A2849] font-mono text-xs uppercase tracking-[0.25em] block mb-2 font-bold">The Artisans</span>
          <h1 className="font-cinzel font-black text-3xl sm:text-6xl text-[#14161C] uppercase tracking-tight break-words">Master Craftsmen</h1>
          <p className="text-gray-600 text-xs sm:text-sm font-mono mt-3">Book directly with your preferred master barber or walk in any day from 7 AM to 7 PM.</p>
        </div>
      </FadeIn>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-8">
        ...
        {barbers.map((barber, index) => (
          <div key={index} className="bg-[#FAF8F3] border border-black/10 rounded-lg overflow-hidden flex flex-col sm:flex-row justify-between group hover:border-[#1A2849] transition-all shadow-sm">
            ...
            <div className="sm:w-2/5 relative h-64 sm:h-auto min-h-[230px] bg-black overflow-hidden shrink-0">
              ...
              />
              <span className="absolute top-3 left-3 px-2 py-0.5 bg-black/85 border border-[#DFBA68]/60 text-[#DFBA68] text-[9px] font-mono uppercase tracking-wider rounded-sm">
                {barber.experience}
              </span>
            </div>
            ...
            <div className="sm:w-3/5 p-5 sm:p-6 flex flex-col justify-between">
              <div>
                <h3 className="font-cinzel font-bold text-xl sm:text-2xl text-[#14161C] mb-1">{barber.name}</h3>
                <p className="text-xs font-mono text-[#1A2849] mb-3 font-semibold">{barber.title}</p>
                
                {/* Tags */}
                <div className="flex flex-wrap gap-1.5 mb-4">
                  {barber.tags.map((tag, tIndex) => (
                    <span key={tIndex} className="px-2 py-0.5 bg-white border border-black/10 text-[9.5px] font-mono text-gray-700 rounded">
                      {tag}
                    </span>
                  ))}
                </div>
              ...
              <a 
                ...
                className="btn-editorial-navy w-full py-2.5 text-xs text-center font-mono"
              >
