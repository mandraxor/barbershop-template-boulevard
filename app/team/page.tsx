...
export const metadata = {
  title: 'Master Craftsmen Roster | Boulevard Barber Shop',
...
export default function TeamPage() {
  ...
  const barbers = [
    {
      name: "Elier",
      title: "Master Barber & Razor Specialist",
    ...
    },
    {
      ...
      title: "Senior Barber & Beard Sculptor",
  ...
  return (
    <main className="py-12 sm:py-20 px-3 sm:px-8 max-w-7xl mx-auto overflow-hidden">
      ...
      <div className="text-center max-w-3xl mx-auto mb-12 sm:mb-16">
        ...
        <h1 className="font-serif font-black text-3xl sm:text-6xl text-white uppercase tracking-tight break-words">Master Craftsmen</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-8">
        ...
        {barbers.map((barber, index) => (
          <div key={index} className="bg-[#121620] border border-[#C5A059]/30 rounded-lg overflow-hidden flex flex-col sm:flex-row justify-between group hover:border-[#C5A059] transition-all">
            
            {/* Portrait Image */}
            <div className="sm:w-2/5 relative h-60 sm:h-auto min-h-[220px] bg-black overflow-hidden shrink-0">
              <img 
                src={barber.image} 
                alt={barber.name} 
                className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500"
              />
              <span className="absolute top-3 left-3 px-2 py-0.5 bg-black/80 border border-[#C5A059]/60 text-[#C5A059] text-[9px] font-mono uppercase tracking-wider rounded">
                {barber.experience}
              </span>
            </div>
            ...
            <div className="sm:w-3/5 p-5 sm:p-6 flex flex-col justify-between">
              <div>
                <h3 className="font-serif font-bold text-xl sm:text-2xl text-white mb-1">{barber.name}</h3>
                <p className="text-xs font-mono text-[#E5C478] mb-3">{barber.title}</p>
                
                {/* Tags */}
                <div className="flex flex-wrap gap-1.5 mb-4">
                  {barber.tags.map((tag, tIndex) => (
                    <span key={tIndex} className="px-2 py-0.5 bg-[#080a0f] border border-white/10 text-[9.5px] font-mono text-gray-300 rounded">
                      {tag}
                    </span>
                  ))}
                </div>
